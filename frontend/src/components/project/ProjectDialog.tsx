...
/** Create a new project (name + color). Repo linking and edit/delete live in Project settings. */
export function ProjectDialog({ open, onOpenChange }: {
  ...
  function submit() {
    if (!name.trim()) return;
    create.mutate({ name: name.trim(), color }, { onSuccess: () => onOpenChange(false) });
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <Button onClick={submit} disabled={!name.trim() || create.isPending}>Create project</Button>
      </DialogContent>
    </Dialog>
  );
}
