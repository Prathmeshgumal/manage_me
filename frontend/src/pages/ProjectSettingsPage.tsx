    ...
    <div className="max-w-3xl mx-auto p-6 flex flex-col gap-6">
      ...
      <section className="rounded-lg border border-border p-4 flex flex-col gap-3 bg-surface">
        ...
        {hasInstalls ? (
          ...
          <p className="text-xs text-ink-muted">Install the app on GitHub (Connect GitHub) to link a repository.</p>
        )}
      </section>
      ...
      {/* Danger */}
      <section className="rounded-lg border border-destructive/40 p-4 flex items-center justify-between">
        <div>
          <div className="text-sm font-medium">Delete project</div>
          <div className="text-xs text-ink-muted">Tasks in this project are kept (unassigned).</div>
        </div>
        <Button variant="destructive" onClick={() => del.mutate(project.id, { onSuccess: onDeleted })} disabled={del.isPending}>
          Delete
        </Button>
      </section>
    </div>
